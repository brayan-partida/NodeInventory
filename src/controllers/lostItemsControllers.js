...
constroller.insertControllers = async (req, res) => {
    ...
    const insertdata = await pool.query(lostItem.insertLost(idLostItem, dateLost, id_Loans));
...
constroller.getUsuarioLostFecha = async (req, res) => {
    ...
    const obtenerItemPerdido = await pool.query(lostItem.obtenerItemPerdidoFecha(dateLost));
    res.json(obtenerItemPerdido);
};

constroller.deleteLostItem = async (req, res) => {
    const consulta = ({idLostItem} = req.params);
    const obtenerItemPerdido = await pool.query("delete from lost_items where idLostItem=" + idLostItem + "");
